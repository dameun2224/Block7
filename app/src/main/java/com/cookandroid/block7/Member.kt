...
class Member(GameActivity: GameActivity, name: String, nameKor: String, memberView: ImageView) {
    ...
    // 상태변화 - 특수
    private var isAlien: Boolean = false // 은주만 가능
    ...
    fun stateChangeVeryCrazy() { isCrazy = true; isVeryCrazy = true }
    fun stateChangeHurt() { isHurt = true }
    fun stateChangeVeryHurt() { isHurt = true; isVeryHurt = true }
    fun stateChangeSick() { isSick = true }
    fun stateChangeVerySick() { isSick = true; isVerySick = true }
    fun stateChangeTired() { isTired = true }
    fun stateChangeFatigued() { isTired = true; isFatigued = true }
    ...
    fun updateDate() {
        ...
        if (!isIn && isAlive) dateExplore++ // 안에 있지 않다면, dateExplore++
    }
